#ifndef ENCODINGS_ROOT_SRC_BENCH_SSB_SCANS_BIT_H_
#define ENCODINGS_ROOT_SRC_BENCH_SSB_SCANS_BIT_H_
...
class SsbWideTableHelperBit {
 public:
  lib::BitStratifiedColumn lo_quantity;
  lib::BitStratifiedColumn lo_extendedprice;
  lib::BitStratifiedColumn lo_discount;
  lib::BitStratifiedColumn lo_revenue;
  lib::BitStratifiedColumn lo_supplycost;
  lib::BitStratifiedColumn p_mfgr;
  lib::BitStratifiedColumn p_category;
  lib::BitStratifiedColumn p_brand1;
  lib::BitStratifiedColumn s_city;
  lib::BitStratifiedColumn s_nation;
  lib::BitStratifiedColumn s_region;
  lib::BitStratifiedColumn c_city;
  lib::BitStratifiedColumn c_nation;
  lib::BitStratifiedColumn c_region;
  lib::BitStratifiedColumn d_year;
  lib::BitStratifiedColumn d_yearmonthnum;
  lib::BitStratifiedColumn d_yearmonth;
  lib::BitStratifiedColumn d_weeknuminyear;
  const lib::SsbWideTable &table_ref;

  inline SsbWideTableHelperBit() = delete;
  inline SsbWideTableHelperBit(SsbWideTableHelperBit const &) = delete;
  void operator=(SsbWideTableHelperBit const &) = delete;

  inline explicit SsbWideTableHelperBit(
      const lib::SsbWideTable &table, const size_t size) : lo_quantity(lib::BitStratifiedColumn(size, 32)),
                                                           lo_extendedprice(lib::BitStratifiedColumn(size, 32)),
                                                           lo_discount(lib::BitStratifiedColumn(size, 32)),
                                                           lo_revenue(lib::BitStratifiedColumn(size, 32)),
                                                           lo_supplycost(lib::BitStratifiedColumn(size, 32)),
                                                           p_mfgr(lib::BitStratifiedColumn(size, 32)),
                                                           p_category(lib::BitStratifiedColumn(size, 32)),
                                                           p_brand1(lib::BitStratifiedColumn(size, 32)),
                                                           s_city(lib::BitStratifiedColumn(size, 32)),
                                                           s_nation(lib::BitStratifiedColumn(size, 32)),
                                                           s_region(lib::BitStratifiedColumn(size, 32)),
                                                           c_city(lib::BitStratifiedColumn(size, 32)),
                                                           c_nation(lib::BitStratifiedColumn(size, 32)),
                                                           c_region(lib::BitStratifiedColumn(size, 32)),
                                                           d_year(lib::BitStratifiedColumn(size, 32)),
                                                           d_yearmonthnum(lib::BitStratifiedColumn(size, 32)),
                                                           d_yearmonth(lib::BitStratifiedColumn(size, 32)),
                                                           d_weeknuminyear(lib::BitStratifiedColumn(size, 32)),
                                                           table_ref(table) {}
...
inline void ssb_scans_q11_bit(
    const lib::SsbWideTable &table,
    const SsbWideTableHelperBit &helper,
...
template<class T, lib::Encoding encoding>
inline void ssb_run_queries_bit(const lib::SsbWideTable &table, const std::string &experiment, size_t repeats) {
  SsbWideTableHelperBit helper = SsbWideTableHelperBit(table, table.n());
  helper.load_helper<T, encoding>(table);
  for (size_t i = 0; i < repeats; i++) {
    ssb_scans_q11_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q11-bit-" + experiment);
    ssb_scans_q11_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q11-bit-" + experiment);
    ssb_scans_q12_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q12-bit-" + experiment);
    ssb_scans_q12_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q12-bit-" + experiment);
    ssb_scans_q13_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q13-bit-" + experiment);
    ssb_scans_q13_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q13-bit-" + experiment);
    ssb_scans_q21_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q21-bit-" + experiment);
    ssb_scans_q21_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q21-bit-" + experiment);
    ssb_scans_q22_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q22-bit-" + experiment);
    ssb_scans_q22_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q22-bit-" + experiment);
    ssb_scans_q23_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q23-bit-" + experiment);
    ssb_scans_q23_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q23-bit-" + experiment);
    ssb_scans_q31_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q31-bit-" + experiment);
    ssb_scans_q31_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q31-bit-" + experiment);
    ssb_scans_q32_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q32-bit-" + experiment);
    ssb_scans_q32_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q32-bit-" + experiment);
    ssb_scans_q33_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q33-bit-" + experiment);
    ssb_scans_q33_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q33-bit-" + experiment);
    ssb_scans_q34_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q34-bit-" + experiment);
    ssb_scans_q34_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q34-bit-" + experiment);
    ssb_scans_q41_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q41-bit-" + experiment);
    ssb_scans_q41_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q41-bit-" + experiment);
    ssb_scans_q42_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q42-bit-" + experiment);
    ssb_scans_q42_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q42-bit-" + experiment);
    ssb_scans_q43_bit<T, encoding, true>(table, helper, std::to_string(i) + ",ssb-q43-bit-" + experiment);
    ssb_scans_q43_bit<T, encoding, false>(table, helper, std::to_string(i) + ",ssb-q43-bit-" + experiment);
  }
}

inline void ssb_wrapper_bit(const lib::SsbWideTable &table, size_t repeats) {
  ssb_run_queries_bit<uint32_t, lib::NONE>(table, "natural", repeats);
  ssb_run_queries_bit<uint32_t, lib::DFE>(table, "dfe", repeats);
  ssb_run_queries_bit<int32_t, lib::EDFE>(table, "edfe", repeats);
}

}// namespace enc::bench

#endif//ENCODINGS_ROOT_SRC_BENCH_SSB_SCANS_BIT_H_
